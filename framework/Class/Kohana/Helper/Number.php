...
class Kohana_Helper_Number {
	 ...
	 *
	 *     echo 2, Helper_Number::ordinal(2);   // "2nd"
	 *     echo 10, Helper_Number::ordinal(10); // "10th"
	 *     echo 33, Helper_Number::ordinal(33); // "33rd"
	 *
	 ...
	 *     // 西班牙格式, "1200,05"
	 *     // 葡萄牙格式, "1 200,05"
	 *     echo Helper_Number::format(1200.05, 2);
	 *
				...
				case self::ROUND_HALF_ODD:
					...
					if (($value * $factor) - floor($value * $factor) === 0.5)
					{
						$up = ($mode === self::ROUND_HALF_DOWN)
							...
							: ( ! ( ! (floor($value * $factor) & 1)) === ($mode === self::ROUND_HALF_EVEN));

						$value = $up ? ceil($value * $factor) : floor($value * $factor);
						return $value / $factor;
					}
					else
					{
	 ...
	 * Helper_Number::$byte_units
	 *
	 *     echo Helper_Number::bytes('200K');  // 204800
	 *     echo Helper_Number::bytes('5MiB');  // 5242880
	 *     echo Helper_Number::bytes('1000');  // 1000
	 *     echo Helper_Number::bytes('2.5GB'); // 2684354560
	 *
	...
	public static function bytes($size)
	{
		...
		$accepted = implode('|', array_keys(Helper_Number::$byte_units));
		...
		$unit = Helper_Array::get($matches, 2, 'B');

		// 把大小转换为字节数
		$bytes = $size * pow(2, Helper_Number::$byte_units[$unit]);
