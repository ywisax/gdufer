...
class Kohana_Route {
	 ...
	 * 检查指定名称的路由是否存在
	 *
	 *     $result = Route::exist('default');
	 *
	 * @param   string  $name   指定的路由名称
	 * @return  Route
	 */
	public static function exist($name)
	{
		return (bool) isset(Route::$_routes[$name]);
	}
	
	/**
	 * test
	 */
	public static function anchor($title, $route, $params = array())
	{
		return HTML::anchor(Route::url($route, $params), $title);
	}
	...
	// 允许访问的协议，默认只能get和post
	protected $_protocol = array('GET', 'POST');
	...
	public function matches(Request $request)
	{
		...
		// 检查是否符合协议的检查
		if ( ! in_array($request->method(), $this->protocol()))
		{
			return FALSE;
		}
	...
	{
		return ! in_array(Helper_Array::get($this->_defaults, 'host', FALSE), Route::$localhosts);
	}

	const URI_PARAMS_REGEX = '#\([^()]++\)#';
	const URI_TRIM_SLASHES_REGEX = '#//+#';
	const URI_TRIM_SLASHES_REPLACE = '/';
	...
	public function uri(array $params = NULL)
	{
		...
		if (strpos($uri, '<') === FALSE AND strpos($uri, '(') === FALSE)
		{
			...
			$params['host'] = (strpos($this->_defaults['host'], '://') === FALSE) // If the localhost setting does not have a protocol
				? Route::$default_protocol.$this->_defaults['host'] // Use the default defined protocol
				: $this->_defaults['host']; // Use the supplied host with protocol
		...
		while (preg_match(Route::URI_PARAMS_REGEX, $uri, $match))
		{
			...
			while (preg_match('#'.Route::REGEX_KEY.'#', $replace, $match))
			{
				...
				if (isset($params[$param]) AND $params[$param] !== Helper_Array::get($this->_defaults, $param))
				{
		...
		$uri = preg_replace(Route::URI_TRIM_SLASHES_REGEX, Route::URI_TRIM_SLASHES_REPLACE, rtrim($uri, '/'));
