...
class Kohana_Validation implements ArrayAccess {
	...
	const RULE_LABEL_MATCH = '/[^\pL]+/u';
	const RULE_LABEL_REPLACE = ' ';
	...
	public function rule($field, $rule, array $params = NULL)
	{
		...
		if ($field !== TRUE AND ! isset($this->_labels[$field]))
		{
			...
			$this->_labels[$field] = preg_replace(Validation::RULE_LABEL_MATCH, Validation::RULE_LABEL_REPLACE, $field);
		}
	...
	public function rules($field, array $rules)
	{
		...
		{
			$this->rule($field, $rule[0], Helper_Array::get($rule, 1));
		}
	...
	public function check()
	{
		...
		$expected = Helper_Array::merge(array_keys($original), array_keys($this->_labels));
		...
		foreach ($expected AS $field)
		{
			...
			$data[$field] = Helper_Array::get($this, $field);
		...
		foreach ($this->_errors AS $field => $set)
		{
			...
			if ($translate)
			{
				$label = is_string($translate)
					? __($label, NULL, $translate) // Translate the label using the specified language
					: __($label); // Translate the label
			}

			// Start the translation values list
			$values = array(
				':field' => $label,
				':value' => Helper_Array::get($this, $field),
			);

			if (is_array($values[':value']))
			{
				// All values must be strings
				$values[':value'] = implode(', ', Helper_Array::flatten($values[':value']));
			}
			...
			if ($params)
			{
				foreach ($params AS $key => $value)
				{
					...
					{
						// All values must be strings
						$value = implode(', ', Helper_Array::flatten($value));
					}
					...
					if (isset($this->_labels[$value]))
					{
						...
						if ($translate)
						{
							$value = is_string($translate)
								? __($value, NULL, $translate) // Translate the value using the specified language
								: __($value); // Translate the value
						}
					}
			...
			if ($translate)
			{
				$message = is_string($translate)
					? __($message, $values, $translate) // Translate the message using specified language
					: __($message, $values); // Translate the message using the default language
			}
			else
			{
