 ...
 * have been collected, load the `Profiler/Stat` [View]:
 *
...
class Kohana_Profiler {
	 ...
	 * 渲染引擎，默认是渲染HTML
	 */
	protected static $_render_engine = 'HTML';
	...
	public static function render_engine($engine = NULL)
	{
		if ($engine === NULL)
		{
			return Profiler::$_render_engine;
		}
		Profiler::$_render_engine = (string) $engine;
	}
	
	const RENDER_ENGINE_PRE_CLASS = 'Profiler_Engine_';

	/**
	 * 获取并返回最后的Profile结果
	 */
	public static function render($engine = NULL)
	{
		// 设定引擎
		if ($engine === NULL)
		{
			$engine = Profiler::render_engine();
		}
	
		// 获取类名
		$class = Profiler::RENDER_ENGINE_PRE_CLASS . $engine;
		$object = new $class;
