...
class Kohana_Response implements HTTP_Response {
	...
	public static $messages = array(
		...
		// SAE接口中新增的几个
		600 => 'Internal Error', //内部错误
		601 => 'No Such User', //_AccessKey_不存在
		602 => 'Auth Error', //认证错误，可能是secretkey 不正确
		603 => 'Out of Quota', //超出fetchurl 的使用配额
		604 => 'REST Error', //REST 协议错误，相关的header 不存在或其它错误
		605 => 'Invalid URI', //_FetchUrl_不是有效的Url
		606 => 'Invalid Host', //_FetchUrl_对应的服务器不可达或者是一个私网地址
		607 => 'Service Not Enabled', //_FetchUrl_对应的扩展服务尚未启用
	);
	...
	public function __construct(array $config = array())
   {
	...
	public function headers($key = NULL, $value = NULL)
	{
		...
		elseif ($value === NULL)
		{
			return Helper_Array::get($this->_header, $key);
		}
		else
		{
	...
	public function cookie($key = NULL, $value = NULL)
	{
		...
		elseif ( ! is_array($key) AND ! $value)
			return Helper_Array::get($this->_cookies, $key);
		...
		else
		{
			...
			{
				$value['expiration'] = Helper_Cookie::$expiration;
			}
	...
	public function send_file($filename, $download = NULL, array $options = NULL)
	{
      ...
      if ($filename === TRUE)
      {
         ...
         if ( ! isset($mime))
         {
            ...
            $mime = Helper_File::mime_by_ext(strtolower(pathinfo($download, PATHINFO_EXTENSION)));
         }
      ...
      else
      {
		...
		if ( ! IN_SAE)
		{
         ...
         if ( ! isset($mime))
         {
            ...
            $mime = Helper_File::mime_by_ext(pathinfo($download, PATHINFO_EXTENSION));
         }
      ...
      if ( ! Kohana::$safe_mode AND ! IN_SAE)
      {
   ...
   public function render()
   {
	...
	const PARSE_BYTE_RANGE_REGEX = '/(-?[0-9]++(?:-(?![0-9]++))?)(?:-?([0-9]++))?/';
	...
	protected function _parse_byte_range()
	{
		...
		preg_match_all(Response::PARSE_BYTE_RANGE_REGEX, $_SERVER['HTTP_RANGE'], $matches, PREG_SET_ORDER);
