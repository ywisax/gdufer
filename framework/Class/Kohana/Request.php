...
/**
 * 核心请求类。使用[Route]类来决定执行的[Controller]。
 *
 * @package    Kohana
 * @category   Request
 */
class Kohana_Request implements HTTP_Request {
	...
	/**
	 * 根据URI来创建请求对象
	 * 为了规范，建议使用[Request::instance]或[Request::factory]来创建对象
	 *
	 *     $request = Request::factory($uri);
	 *
	 * 如果设置了`$cache`，那么对象会首先尝试从缓存中读取输出
	 *
	 * @param   string  $uri              请求URI
	 * @param   array   $client_params    传递给Client的参数数组
	...
	public static function factory($uri = TRUE, $client_params = array(), $injected_routes = array())
	{
		...
		if ( ! Request::$initial)
		{
			...
			$method = isset($_SERVER['REQUEST_METHOD'])
				? $_SERVER['REQUEST_METHOD'] // 使用发送过来的请求类型
				: HTTP_Request::GET; // 默认为GET请求
			...
			if (($cookie_keys = array_keys($_COOKIE)))
			{
				...
				{
					$cookies[$key] = Helper_Cookie::get($key);
				}
			}
	...
	{
		return Helper_Text::user_agent(Request::$user_agent, $value);
	}
	 ...
	 * 获取或设置HTTP请求字符串
	 *
	 * @param   mixed   $key    键名或者要设置的数组
	 * @param   string  $value  要传递的值
	 * @return  mixed
	 */
	public function query($key = NULL, $value = NULL)
	{
		...
		elseif ($value === NULL)
		{
			...
			return Helper_Array::path($this->_get, $key);
		}
	...
	public function post($key = NULL, $value = NULL)
	{
		...
		elseif ($value === NULL)
		{
			...
			return Helper_Array::path($this->_post, $key);
		}
