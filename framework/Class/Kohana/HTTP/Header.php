...
/**
 * Kohana_HTTP_Header提供了一个面向对象的HTTP头信息控制类。
 * 它可以解析从`apache_request_headers()`或`http_parse_headers()`返回的头部信息数组。
 *
 * @package    Kohana
 * @category   HTTP
 */
class Kohana_HTTP_Header extends ArrayObject {
	...
	const ACCEPT_QUALITY_REGEX = '~\b(\;\s*+)?q\s*+=\s*+([.0-9]+)~';
	...
	public static function accept_quality(array $parts)
	{
		...
		foreach ($parts_keys AS $key)
		{
			...
			if ( ! preg_match(HTTP_Header::ACCEPT_QUALITY_REGEX, $value, $quality))
			{
			...
			else
			{
				...
				$parsed[trim(preg_replace(HTTP_Header::ACCEPT_QUALITY_REGEX, '', $value, 1), '; ')] = (float) $quality;
			}
		}
	...
	public function parse_header_string($resource, $header_line)
	{
		...
		if (preg_match_all(HTTP_Header::PARSE_HEADER_STRING, $header_line, $matches))
		{
	...
	public function send_headers(HTTP_Response $response = NULL, $replace = FALSE, $callback = NULL)
	{
		...
		foreach ($headers AS $header => $value)
		{
			...
			$processed_headers[] = Helper_Text::ucfirst($header).': '.$value;
	...
	protected function _send_headers_to_php(array $headers, $replace)
	{
		...
		foreach ($headers AS $key => $line)
		{
			if ($key == 'Set-Cookie' AND is_array($line))
			{
				...
				{
					Helper_Cookie::set($name, $value['value'], $value['expiration']);
				}
