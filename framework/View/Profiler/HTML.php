...
<div class="kohana">
	...
	<table class="profiler">
		...
		<tr class="final mark time">
			...
			<td class="<?php echo $key ?>"><?php echo number_format($stats[$key]['time'], 6) ?> <abbr title="<?php echo __('seconds') ?>">s</abbr></td>
			<?php endforeach ?>
		</tr>
		<tr class="final mark memory">
			<?php foreach ($application_cols AS $key): ?>
			<td class="<?php echo $key ?>"><?php echo number_format($stats[$key]['memory'] / 1024, 4) ?> <abbr title="<?php echo __('kilobyte') ?>">kB</abbr></td>
			<?php endforeach; ?>
		</tr>
	</table>
	...
	<?php foreach (array('_SESSION', '_GET', '_POST', '_FILES', '_COOKIE') AS $var): ?>
		...
		<tr class="final mark">
			<th class="name" colspan="2"><?php echo $var; ?></th>
		</tr>
		<?php foreach ($GLOBALS[$var] AS $key => $value): ?>
		<tr>
			<td style="text-align:right;"><code><?php echo UTF8::clean($key) ?></code></td>
			<td style="text-align:left;"><pre><?php echo Debug::dump($value) ?></pre></td>
		</tr>
		<?php endforeach; ?>
	<?php endforeach ?>
	</table>
	...
	<?php $included = get_included_files() ?>
		<tr class="final mark">
			<th class="name" colspan="2"><?php echo __('Included files') ?></a> (<?php echo count($included) ?>)</th>
		</tr>
		<?php foreach ($included AS $file): ?>
		<tr>
			<td style="text-align:left;"><code><?php echo Debug::path($file) ?></code></td>
		</tr>
	<?php endforeach ?>
	</table>
	...
	<table class="profiler">
		...
		<tr class="final mark">
			<th class="name" colspan="2"><?php echo __('Loaded extensions') ?></a> (<?php echo count($included) ?>)</th>
		</tr>
		<?php foreach ($included AS $file): ?>
		<tr>
			<td style="text-align:left;"><code><?php echo Debug::path($file) ?></code></td>
