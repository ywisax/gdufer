...
To load a config group simply call `Kohana::config()` with the name of the group you wish to load:

	$config = Kohana::config('my_group');
...
from the config group to the value you want:
	...
	$hostname = Kohana::config('database.default.connection.hostname');


Which is equivalent to:

	$config = Kohana::config('Database')->get('default');
...
For example, using the setup outlined above:
	...
	// Configuration returned by Kohana::config('email')
		email:
			sender:
				email: my.supercool.address@gmail.com
				name:  Kohana Bot
