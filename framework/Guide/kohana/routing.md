...
Kohana有一个强大的路由系统。
从本质来说，路由系统提供了一个URL跟控制器与动作之间交流的接口。
设置了一个正确的路由，你除了可以自定义一个个性化的URL，还可以通过URL给控制器和动作传递特定的参数，实现各种功能。
...
一般来说，在`APP_PATH/Init.php`或者`SYS_PATH/Init.php`，你可以找到一个第一个参数为"default"的路由：
...
[!!] 默认的路由只是提供来做为例子的，你可以删除它或者替换为你自己的更强大的路由。
...
The established convention is to either place your custom routes in the `MOD_PATH/<module>/Init.php` file of your module if the routes belong to a module, or simply insert them into the `APP_PATH/Init.php` file (be sure to put them **above** the default route) if they are specific to the application. Of course, nothing stops you from including them from an external file, or even generating them dynamically.
