...
First, we need a [View] that contains the HTML form, which will be placed in `APP_PATH/View/user/register.php`:
...
Next, we need a controller and action to process the registration, which will be placed in `APP_PATH/Class/Controller/User.php`:
...
We will also need a user model, which will be placed in `APP_PATH/Class/Model/User.php`:
