...
Kohana提供了不少助手类，让我们可以更高效地开发。

助手类同样放置在`Class`目录中即可，跟正常的类一样，可以继承和扩展。

# 常用助手

下面简单介绍了常用的助手类：

 - **[Helper_Array]** - Array functions.  Get an array key or default to a set value, get an array key by path, etc.
