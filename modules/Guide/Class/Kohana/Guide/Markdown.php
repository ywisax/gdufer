 ...
 */
class Kohana_Guide_Markdown extends Markdown_Extra {
	...
	/**
	 * Transform some text using [Guide_Markdown]
	 *
	 * @see Markdown()
	 *
	...
	public function doBaseURL($text)
	{
		...
		return preg_replace('~(?<!!)(\[.+?\]\()(?!\w++://)(?!#)(\S*(?:\s*+".+?")?\))~', '$1'.Guide_Markdown::$base_url.'$2', $text);
	}
	...
	{
		// URLs containing "://" are left untouched
		return preg_replace('~(!\[.+?\]\()(?!\w++://)(\S*(?:\s*+".+?")?\))~', '$1'.Guide_Markdown::$image_url.'$2', $text);
	}
	...
	{
		return preg_replace_callback('/\['.Guide::$regex_class_member.'\]/i', 'Guide::link_class_member', $text);
	}
	...
	public function doTOC($text)
	{
		...
		if (self::$show_toc AND Route::name(Request::current()->route()) == 'guide-doc')
		{
			$toc = View::factory('Guide.PageToc')
				->set('array', Guide_Markdown::$_toc)
				->render()
				;
