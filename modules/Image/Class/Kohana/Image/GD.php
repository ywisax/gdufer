...
class Kohana_Image_GD extends Image {
	...
	const GD_VERSION_CHECKER_REGEX = '/\d+\.\d+(?:\.\d+)?/';
	const GD_BUNDLED_REGEX = '/\bbundled\b/i';
	...
	public static function check()
	{
		...
		else
		{
			...
			Image_GD::$_bundled = (bool) preg_match(Image_GD::GD_BUNDLED_REGEX, $info['GD Version']);
		}
		...
		else
		{
			...
			preg_match(Image_GD::GD_VERSION_CHECKER_REGEX, $info['GD Version'], $matches);
	...
	protected function _do_reflection($height, $opacity, $fade_in)
	{
		...
		$stepping = ($opacity < 127)
			? ((127 - $opacity) / $height) // Calculate the opacity stepping
			: (127 / $height); // Avoid a "divide by zero" error
