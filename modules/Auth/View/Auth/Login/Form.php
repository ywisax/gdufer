<form action="<?php echo Route::url('auth-action', array('action' => 'login')) ?>" method="post" class="form-horizontal<?php if (isset($ajax) AND $ajax) { echo ' ajax'; } else {  /*echo ' well';*/ } ?> login-form">
	<fieldset>
		<?php if (isset($ajax) AND $ajax): ?>
		<?php else: ?>
		<legend>
			<?php echo __('User Login') ?>
			<small class="pull-right"><?php echo HTML::anchor(Route::url('auth-action', array('action' => 'register')), __('Sign up')) ?></small>
		</legend>
		<?php endif; ?>
		<div class="control-group<?php if (isset($errors['username'])): ?> error<?php endif; ?>">
			...
			<div class="controls">
				<input id="username" type="text" name="username" placeholder="<?php echo __('Input your username/email') ?>" class="input-large" value="<?php if (isset($username)) { echo $username; } ?>" />
		...
		<div class="control-group">
			<div class="controls">
				...
				<a href="<?php echo Route::url('auth-action', array('action' => 'reset')) ?>" class="btn btn-secondary"><?php echo __('Lost Password') ?></a>
			</div>
		</div>
	</fieldset>
</form>
