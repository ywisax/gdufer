...
class Kohana_Auth_ORM extends Auth {
	...
	protected function _login($user, $password, $remember)
	{
		...
		if ($user->has('roles', ORM::factory('role', array('name' => 'login'))) AND $user->password === $password)
		{
			if ($remember === TRUE)
			{
				...
				Helper_Cookie::set('authautologin', $token->token, $this->_config['lifetime']);
			}
	...
	{
		if ($token = Helper_Cookie::get('authautologin'))
		{
			...
			if ($token->loaded() AND $token->user->loaded())
			{
				if ($token->user_agent === sha1(Request::$user_agent))
				{
					...
					$token->save();
					// Set the new token
					Helper_Cookie::set('authautologin', $token->token, $token->expires - time());
	...
	public function logout($destroy = FALSE, $logout_all = FALSE)
	{
		...
		if ($token = Helper_Cookie::get('authautologin'))
		{
			...
			Helper_Cookie::delete('authautologin');
