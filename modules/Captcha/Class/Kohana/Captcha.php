...
abstract class Kohana_Captcha
{
	...
	const DEFAULT_CLASS_PREFIX = 'Captcha_';
	...
	public static function instance($group = 'default')
	{
		if ( ! isset(Captcha::$instance))
		{
			...
			// 设置类名
			$class = Captcha::DEFAULT_CLASS_PREFIX . ucfirst($config['style']);
			// 新建实例
	...
	public function __construct($group = NULL)
	{
		...
		{
			throw new Kohana_Exception('Captcha group not defined in :group configuration', array(
				':group' => $group
			));
		}
	...
	public static function valid($response)
	{
		...
		//$result = (bool) (strtoupper($response) === Session::instance()->get('captcha_response'));
	...
	public function image_render($html)
	{
		...
		if ($html === TRUE)
		{
			//$url = URL::site('captcha/' . Captcha::$config['group']) . '?code=' . Helper_Text::random('alnum', 32);
			$url = URL::site('captcha/' . Captcha::$config['group']);
			return '<img src="'.$url.'" width="'.Captcha::$config['width'].'" height="'.Captcha::$config['height'].'" style="cursor:pointer;" onclick="this.src=this.src+\'?\'" alt="' . __('Click to refresh') . '" class="captcha" />';
		}
