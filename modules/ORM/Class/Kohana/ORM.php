...
class Kohana_ORM extends Model implements serializable {
	...
	/**
	 * @var  array  数据库直接调用的方法
	 */
	protected static $_db_methods = array(
		'where', // and_where()的别名
		'and_where', // Creates a new "AND WHERE" condition for the query.
		'or_where', // Creates a new "OR WHERE" condition for the query.

		'where_open',
		'and_where_open', // Opens a new "AND WHERE (...)" grouping.
		'or_where_open', // Opens a new "OR WHERE (...)" grouping.

		//'where_close',
		'and_where_close', // Closes an open "AND WHERE (...)" grouping.
		'or_where_close', // Closes an open "OR WHERE (...)" grouping.

		'distinct', // Enables or disables selecting only unique columns using "SELECT DISTINCT"
		...
		'group_by', // Creates a "GROUP BY ..." filter.
		'having',
		'and_having', // Creates a new "AND HAVING" condition for the query.
		'or_having', // Creates a new "OR HAVING" condition for the query.

		//'having_open',
		'and_having_open', // Opens a new "AND HAVING (...)" grouping.
		'or_having_open', // Opens a new "OR HAVING (...)" grouping.

		//'having_close',
		'and_having_close', // Closes an open "AND HAVING (...)" grouping.
		'or_having_close', // Closes an open "OR HAVING (...)" grouping.

		'order_by',
		'limit', // Return up to "LIMIT ..." results
		'offset', // Start returning results after "OFFSET ..."

		'cached', // Applies sorting with "ORDER BY ..."
		'using', // Adds "USING ..." conditions for the last created JOIN statement.
	...
	protected function _initialize()
	{
		...
		if ( ! $init = Helper_Array::get(ORM::$_init_cache, $this->_object_name, FALSE))
		{
			$init = array(
				'_belongs_to' => array(),
				'_has_one'    => array(),
			...
			{
				$init['_object_plural'] = Helper_Inflector::plural($this->_object_name);
			}
			...
			if (empty($this->_table_name))
			{
				...
				if ($this->_table_names_plural === TRUE)
				{
					...
					$init['_table_name'] = Helper_Array::get($init, '_object_plural', $this->_object_plural);
				}
			}
			...
			foreach ($this->_has_many AS $alias => $details)
			{
				if ( ! isset($details['model']))
				{
					$defaults['model'] = str_replace(' ', '_', ucwords(str_replace('_', ' ', Helper_Inflector::singular($alias))));
				}
				...
				{
					$defaults['far_key'] = Helper_Inflector::singular($alias).$this->_foreign_key_suffix;
				}
	...
	public function __call($method, array $args)
	{
		...
		{
			throw new Kohana_Exception('Invalid method :method called in :class', array(
				':method' => $method,
	...
	public function changed($field = NULL)
	{
		return ($field === NULL)
			? $this->_changed
			: Helper_Array::get($this->_changed, $field);
	}
	...
	protected function _load_result($multiple = FALSE)
	{
		...
		else
		{
			...
			($result->count() === 1)
				? $this->_load_values($result->current()) // Load object values
				: $this->clear(); // Clear the object, nothing was found
	...
	protected function run_filter($field, $value)
	{
		...
		foreach ($filters AS $array)
		{
			...
			$filter = $array[0];
			$params = Helper_Array::get($array, 1, array(':value'));
	...
	public function update(Validation $validation = NULL)
	{
		...
		// 唯一字段，不可编辑
		if ( ! empty($this->_uneditable_columns))
		{
			foreach ($this->_uneditable_columns AS $column)
			{
	...
	public function has($alias, $far_keys = NULL)
	{
		...
		return ($far_keys === NULL)
			? (bool) $count
			: ($count === count($far_keys));
